...
- 总量模式单独保存服务商已用量，不猜测上下行分配。
- 首页“流量概览”、节点卡片的上传/下载累计和节点表格显示服务端持久化的总流量。总流量不依赖在线状态，也不受账单周期重置影响；断线重连、服务重启和 Agent 计数器重置都会继续累加。周期已用量、进度条和提醒使用另一组独立数据。后续按节点已保存的计费方式累加增量。例如 sum 计费填写 120 GB，之后上传 1 GB、下载 2 GB，计费已用显示 123 GB；仅上传计费则显示 121 GB。卡片、表格、首页汇总和阈值提醒包含这笔初始总量。
...
- `common:getNodesLatestStatus` 的 `net_total_up/down` 保持原始含义，另提供 `traffic_used_up/down`、按计费规则合计的 `traffic_used_total`、标识总量模式的 `traffic_total_mode`，以及不受周期影响的 `traffic_lifetime_up/down`。默认前端首页卡片、列表、概览和阈值提醒使用周期数值。第三方主题需使用新字段才能显示周期值。
