...
func CheckKomariVersion(constraint string) error {
	...
	// Release hotfix/fork suffixes do not change the numeric plugin API version.
	runningVersion, _, _ := strings.Cut(utils.CurrentVersion, "-")
	runningVersion, _, _ = strings.Cut(runningVersion, "+")
	have, err := parseSemver(runningVersion)
	if err != nil {
