...
func Initialize() error {
	...
	if err := messageSender.RegisterLegacyChannels(); err != nil {
		return err
	}
...
// Shutdown unregisters every built-in notification channel.
func Shutdown() error {
	channels := messageSender.ListNotificationChannels()
	var err error
	for _, item := range channels {
		if unregisterErr := messageSender.UnregisterNotificationChannel(item.ID); unregisterErr != nil && err == nil {
			err = unregisterErr
		}
	}
	return err
