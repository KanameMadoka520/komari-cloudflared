...
func (c *Controller) runLegacy(cfg metricstore.MetricStoreConfig, legacyRetentionDays int) {
	// The streaming source read holds a main-DB connection for the entire import.
	// Keep a second connection available for authenticated progress polling. The
	// guide excludes normal writes; restore the pool limit before normal startup.
	if sqlDB, err := c.db.DB(); err == nil {
		previousLimit := sqlDB.Stats().MaxOpenConnections
		if previousLimit == 1 {
			sqlDB.SetMaxOpenConns(2)
			defer sqlDB.SetMaxOpenConns(previousLimit)
		}
	}
