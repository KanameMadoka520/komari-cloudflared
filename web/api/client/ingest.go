...
func ingestReport(uuid string, report v2.Report, markPresence bool) error {
	return ingestReportWithProtocol(uuid, report, markPresence, true)
}

func ingestReportWithProtocol(uuid string, report v2.Report, markPresence, v2Client bool) error {
	report.UUID = uuid
	...
	agent_runtime.SetClientProtocol(uuid, v2Client)
	if markPresence {
