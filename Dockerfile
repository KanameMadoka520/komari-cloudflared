...
ARG VERSION=1.5.0-fix1-cloudflared.1
...
RUN --mount=type=cache,target=/go/pkg/mod --mount=type=cache,target=/root/.cache/go-build CGO_ENABLED=1 GOOS=${TARGETOS} GOARCH=${TARGETARCH} go build -trimpath \
    -ldflags "-s -w -X github.com/komari-monitor/komari/utils.CurrentVersion=${VERSION} -X github.com/komari-monitor/komari/utils.VersionHash=${BUILD_HASH}" \
    -o /out/komari .
...
ENV GIN_MODE=release KOMARI_DB_TYPE=sqlite KOMARI_DB_FILE=/app/data/komari.db KOMARI_LISTEN=0.0.0.0:25774
EXPOSE 25774
